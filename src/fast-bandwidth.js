        ...
        node.on('input', async function(msg) {
            ...
            try {
                await api({measureUpload: config.upload}).forEach(result => {
                    data = result;
                    node.status({fill: "blue", shape: "dot", text: `fetching... download: ${data.downloadSpeed} ${data.downloadUnit}`});
                });
                msg.payload = data;
                node.status({fill: "green", shape: "dot", text: `done! download: ${data.downloadSpeed} ${data.downloadUnit}`});
            } catch (e) {
                console.error(e);
